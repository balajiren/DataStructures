...
public class SodaFillProblem {
	...
	public static void main(String[] args) {
		...
		ArrayList<String> result = GetSodaCombinations(buttons,1,0,0,1600);
	...
	public static ArrayList<String> GetSodaCombinations(ArrayList<Button> buttons,int currentButtonIndex, int currentmax, int minLevel, int maxLevel)
	{
		...
		for(int i=currentButtonIndex; i<buttons.size()+1;i++)
		{
			if(currentmax + buttons.get(i-1).max < maxLevel)
			{
				currentmax+=buttons.get(i-1).max;
	     	    result.add(buttons.get(i-1).button);
			}
		    
	    
		}
		buttonCombinations.add(result.toString());
	
		GetSodaCombinations(buttons,currentButtonIndex+1,0,minLevel,maxLevel);
